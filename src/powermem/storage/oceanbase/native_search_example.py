...
class OceanBaseVectorStore(VectorStoreBase):
    ...
    def __init__(
        ...
        # Detect OceanBase version and check native hybrid search support
        self.ob_version = self._get_oceanbase_version()
        self.support_native_hybrid = self.ob_version >= (4, 4, 1)
        logger.info(
            f"OceanBase version: {'.'.join(map(str, self.ob_version))}, "
            f"Native hybrid search supported: {self.support_native_hybrid}"
        )
    ...
    def _get_oceanbase_version(self) -> tuple:
        """Get OceanBase version and return as (major, minor, patch) tuple.

        Returns:
            tuple: Version tuple (major, minor, patch), e.g., (4, 4, 1)
                   Returns (0, 0, 0) if version detection fails.
        """
        try:
            with self.obvector.engine.connect() as conn:
                result = conn.execute(text("SELECT OB_VERSION()"))
                version_str = result.scalar()

                if not version_str:
                    logger.warning("OB_VERSION() returned empty, assuming version < 4.4.1")
                    return (0, 0, 0)

                # Parse version string, e.g., "4.4.1.0" -> (4, 4, 1)
                parts = version_str.split('.')
                if len(parts) >= 3:
                    return (int(parts[0]), int(parts[1]), int(parts[2]))
                else:
                    logger.warning(f"Unexpected version format: {version_str}, assuming < 4.4.1")
                    return (0, 0, 0)

        except Exception as e:
            logger.warning(f"Failed to get OceanBase version: {e}, assuming version < 4.4.1")
            return (0, 0, 0)
    ...
    def _configure_vector_index_settings(self):
        ...
        try:
            ...
            version = self._get_oceanbase_version()

            # Set default table organization to heap (only for version >= 4.4.1)
            if version >= (4, 4, 1):
                with self.obvector.engine.connect() as conn:
                    conn.execute(text("ALTER SYSTEM SET default_table_organization = 'heap'"))
                    conn.commit()
                    logger.info("Set default_table_organization = 'heap'")
            else:
                logger.info(
                    f"Skipping default_table_organization setting (OceanBase version {'.'.join(map(str, version))} < 4.4.1)")
    ...
    def _convert_filters_to_native_format(self, filters: Optional[Dict] = None, skip_non_table_fields: bool = False) -> \
    List[Dict]:
        """
        Convert mem0 filter format to OceanBase native DBMS_HYBRID_SEARCH.SEARCH filter format.
        ...
        For fields not in standard columns, uses JSON path notation (metadata.field_name).

        Args:
            filters (Optional[Dict]): The filter conditions in mem0 format.
                Examples:
                - {"user_id": "alice", "rating": {"gte": 4.0}}
                - {"category": ["food", "travel"]}
                - {"AND": [{"user_id": "alice"}, {"OR": [{"rating": {"gte": 4.0}}, {"priority": "high"}]}]}
            skip_non_table_fields (bool): If True, skip filters for fields not in table columns.
                Useful for native hybrid search which doesn't support JSON path filtering.

        Returns:
            List[Dict]: Filter conditions in OceanBase SEARCH native format.
                Examples:
                - [{"term": {"user_id": "alice"}}, {"range": {"rating": {"gte": 4.0}}}]
                - [{"bool": {"should": [{"term": {"category": "food"}}, {"term": {"category": "travel"}}]}}]
        """
        if not filters:
            return []
        ...
        def get_field_name(key: str) -> Optional[str]:
            """
            Get the field name for filter, handling JSON path for metadata fields.

            Args:
                key: Original field name from filter

            Returns:
                Field name to use in filter (may include JSON path like metadata.field_name)
                Returns None if skip_non_table_fields=True and field is not in standard columns
            """
            if key in standard_columns:
                return key
            else:
                # Field is in metadata JSON
                if skip_non_table_fields:
                    # Skip non-table fields when requested
                    logger.debug(f"Skipping non-table field in native hybrid search: {key}")
                    return None
                # Use JSON path notation for metadata fields
                return f"{self.metadata_field}.{key}"

        def process_single_filter(key: str, value: Any) -> Optional[Dict]:
            """Process a single filter condition."""
            # Get the actual field name (handles JSON path for metadata fields)
            field_name = get_field_name(key)

            # Skip if field_name is None (non-table field when skip_non_table_fields=True)
            if field_name is None:
                return None

            # 1. List values -> IN query -> bool.should with multiple term queries
            ...
            if isinstance(value, dict):
                ...
                # nin operator -> bool.must_not with should
                if "nin" in value:
    ...
    def _build_record_for_insert(self, vector: List[float], payload: Dict) -> Dict[str, Any]:
        ...
        if self.include_sparse and "sparse_embedding" in payload:
            record[self.sparse_vector_field] = payload[
                "sparse_embedding"]  # SQLAlchemy JSON type handles serialization automatically
    ...
    def _parse_native_hybrid_results(self, result_json_str: str) -> List[OutputData]:
        """
        ...
        Returns:
            List[OutputData]: Parsed search results
        """
        try:
            # Parse JSON string
            ...
            for doc in result_data:
                # Extract primary key and score
                vector_id = doc.get(self.primary_field)
                score = doc.get("_score", 0.0)

                if not vector_id:
                    logger.warning(f"Document missing primary key '{self.primary_field}', skipping")
                    continue

                # Extract text content
                text_content = doc.get(self.text_field, "")
                ...
                # Extract JSON metadata field
                metadata_json = doc.get(self.metadata_field, {})
                ...
                output_list.append(
                    self._create_output_data(vector_id, text_content, score, metadata)
                )

            logger.info(f"Parsed {len(output_list)} results from native hybrid search")
            return output_list
    ...
    def _native_hybrid_search(
        ...
        """
        Perform hybrid search using OceanBase 4.4.1+ native DBMS_HYBRID_SEARCH.SEARCH.

        This method leverages the database's native hybrid search capabilities to combine
        full-text search and vector search with RRF (Reciprocal Rank Fusion) ranking,
        which is more efficient than application-level fusion.
        ...
        """
        try:
            # 1. Extract and normalize query vector
            ...
            # 2. Convert filters to native format (skip non-table fields for native hybrid search)
            native_filters = self._convert_filters_to_native_format(filters, skip_non_table_fields=True)
            ...
            if native_filters:
                # Add filters to both full-text and vector search
                search_params["query"]["bool"]["filter"] = native_filters
                search_params["knn"]["filter"] = native_filters

            # 4. Execute native hybrid search
            search_params_json = json.dumps(search_params)

            logger.debug(f"Executing native hybrid search on table: {self.collection_name}")
            logger.debug(f"Search parameters: {search_params_json}")

            # DBMS_HYBRID_SEARCH.SEARCH doesn't support parameter binding
            # Use MySQL user variable to pass JSON parameter safely
            with self.obvector.engine.connect() as conn:
                with conn.begin():
                    # Set user variable with the search parameters
                    conn.execute(
                        text("SET @search_params = :params").bindparams(params=search_params_json)
                    )

                    # Call DBMS_HYBRID_SEARCH.SEARCH with table name and user variable
                    # Table name is safe as it's controlled by the application
                    result = conn.execute(
                        text(f"SELECT DBMS_HYBRID_SEARCH.SEARCH('{self.collection_name}', @search_params)")
                    )
                    result_json_str = result.scalar()
            ...
            return self._parse_native_hybrid_results(result_json_str)
    ...
    def _hybrid_search(self, query: str, vectors: List[List[float]], limit: int = 5, filters: Optional[Dict] = None,
        ...
        Automatically selects between native database hybrid search (OceanBase 4.4.1+)
        and application-level fusion based on database version and fusion method.
        ...
        Args:
            ...
            k: RRF rank_constant parameter

        Returns:
            List[OutputData]: Hybrid search results
        """
        # Use native hybrid search if supported and fusion method is RRF
        if self.support_native_hybrid and fusion_method == "rrf":
            try:
                logger.debug("Using OceanBase native hybrid search (DBMS_HYBRID_SEARCH.SEARCH)")
                return self._native_hybrid_search(query, vectors, limit, filters, k)
            except Exception as e:
                logger.warning(f"Native hybrid search failed: {e}, falling back to legacy implementation")
                # Fall through to legacy implementation

        # Use legacy application-level hybrid search
        logger.debug("Using legacy application-level hybrid search")
        return self._legacy_hybrid_search(query, vectors, limit, filters, fusion_method, k)

    def _legacy_hybrid_search(self, query: str, vectors: List[List[float]], limit: int = 5,
                              filters: Optional[Dict] = None,
                              fusion_method: str = "rrf", k: int = 60):
        """
        Legacy hybrid search implementation using application-level fusion.

        This method performs vector search and full-text search in parallel,
        then combines the results using RRF or weighted fusion in the application layer.

        Used for OceanBase versions < 4.4.1 or when fusion_method is "weighted".
        ...
        logger.debug(f"_legacy_hybrid_search results, len : {len(hybrid_results)}")
    ...
    def delete(self, vector_id: int):
        ...
        try:
            ...
            logger.error(f"Failed to delete vector with ID {vector_id} from collection '{self.collection_name}': {e}",
                         exc_info=True)
            raise
    ...
    def update(self, vector_id: int, vector: Optional[List[float]] = None, payload: Optional[Dict] = None):
        ...
        try:
            ...
            logger.error(f"Failed to update vector with ID {vector_id} in collection '{self.collection_name}': {e}",
                         exc_info=True)
            raise
    ...
    def get(self, vector_id: int):
        ...
        try:
            ...
            logger.error(f"Failed to get vector with ID {vector_id} from collection '{self.collection_name}': {e}",
                         exc_info=True)
            raise
