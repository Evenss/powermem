...
class OceanBaseUtil:
    ...
    def create_table_with_index_params(
        ...
        columns: list,
        indexes: list = None,
        vidxs: "IndexParams" = None,
        fts_idxs: list = None,
        partitions: "ObPartition" = None,
        **kwargs,
    ):
        """Create table with optional index_params and additional table options.

        This is an enhanced version of pyobvector's create_table_with_index_params
        that supports additional keyword arguments like mysql_organization='heap'.
        ...
        Args:
            ...
            **kwargs: additional table options (e.g., mysql_organization='heap')
        """
        kwargs.setdefault("extend_existing", True)
        
        # Get sparse vector index params
        sparse_vidxs = None
        if vidxs is not None:
            sparse_vidxs = [v for v in vidxs if v.is_index_type_sparse_vector()]
        ...
        with obvector.engine.connect() as conn:
            with conn.begin():
                ...
                if sparse_vidxs is not None and len(sparse_vidxs) > 0:
                    # Sparse vector indexes need special SQL handling
                    create_table_sql = str(CreateTable(table).compile(obvector.engine))
                    ...
                    table.create(obvector.engine, checkfirst=True)
                ...
                # create vector indexes (excluding sparse)
                if vidxs is not None:
                    for vidx in vidxs:
                        ...
                        vec_idx.create(obvector.engine, checkfirst=True)
                ...
                if fts_idxs is not None:
                    for fts_idx in fts_idxs:
                        ...
                        fts_index.create(obvector.engine, checkfirst=True)
        
        logger.debug(f"Table '{table_name}' created successfully with indexes")